...
- Multicycle RV32I CPU with iterative RV32M and RV32A word operations
- `Zicsr`, `Zifencei`, Machine mode, minimal User mode, and counters
- 8 MiB read-only memory-mapped SPI flash at `0x2000_0000`
- 8 MiB SPI/quad-I/O PSRAM at `0x8000_0000`
- Four-word, one-line instruction prefetch cache
...
- Compact 16-source, single-hart PLIC-compatible interrupt controller
- GPIO and pin multiplexing, UART, SPI, I2C, five PWM channels, two timers,
  and a watchdog
...
iterative to reduce area. RV32A operations use the same single-master memory
port, so their read/modify/write sequence is indivisible relative to the bus
agents present in this design.
...
| Atomics | RV32A word operations: LR.W, SC.W, and all AMOs |
| Privilege | Machine mode and minimal User mode |
| CSRs | Machine trap/interrupt CSRs, `misa`, counters/time |
...
|---|---|---|
| `ui[0]` | Flash MISO | — |
| `ui[1]` | GPIO input | SPI MISO |
| `ui[2:6]` | GPIO inputs | — |
| `ui[7]` | UART RX | GPIO input |
| `uo[0]` | Flash CS# | — |
| `uo[1]` | Flash MOSI | — |
| `uo[2]` | Flash SCK | — |
| `uo[3]` | UART TX | GPIO or PWM0 |
| `uo[4]` | GPIO | SPI CS# or PWM1 |
| `uo[5]` | GPIO | SPI MOSI or PWM2 |
| `uo[6]` | GPIO | SPI SCK or PWM3 |
| `uo[7]` | GPIO | PWM4 |
| `uio[0]` | PSRAM CS# | — |
| `uio[1]` | PSRAM SCK | — |
| `uio[2]` | PSRAM SD0 | — |
...
`clk` is configured for 32 MHz. `rst_n` is asynchronous active-low at the
...
| `0x1000_3000..0x1000_3FFF` | SPI master |
| `0x1000_4000..0x1000_4FFF` | I2C master |
| `0x1000_5000..0x1000_5FFF` | PWM |
| `0x1000_6000..0x1000_60FF` | Timer 0 |
| `0x1000_6100..0x1000_61FF` | Timer 1 |
| `0x1000_7000..0x1000_7FFF` | Watchdog |
...
| `0x08` | FEATURES | bit 0 M, bit 1 counters, bits 7:4 cache words, bit 8 A, bit 9 U-mode |
...
The block implements 16 priorities, pending bits, Machine-mode enables,
threshold, and claim/complete registers at standard PLIC offsets. Interrupt IDs
are: 1 UART, 2 SPI, 3 I2C, 4 GPIO, 5 timer 0, 6 timer 1, 7 watchdog; 8–16 are
reserved. Claimed level sources are held in service until completion.
...
Pin-mux value 0 selects GPIO. Value 1 selects UART/SPI on `uo[3:6]`; value 2
selects PWM. UART TX is selected on `uo[3]` after reset for an early console.
...
No-MMU Linux is experimental. RV32A and User mode preserve a possible path, but
Tiny32 has no Supervisor mode, MMU, PMP, upstream device tree, Linux drivers,
or demonstrated Linux boot. Eight MiB of RAM is also very constrained.

## Building firmware

Install a bare-metal RISC-V toolchain that provides
`riscv64-unknown-elf-gcc`, then run:

```sh
make firmware
```

This builds `software/bringup/flash_image.bin`. To package another flat binary:

```sh
python3 tools/make_boot_flash.py \
  software/boot_stage1/boot_stage1.bin app.bin flash_image.bin \
  --load 0x80000000 --entry 0x80000000
```
