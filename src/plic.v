// -----------------------------------------------------------------------------
// plic.v - compact single-hart, machine-mode PLIC-compatible interrupt block.
// SoC base: 0x0C00_0000
// Supports 16 interrupt sources, IDs 1..16, 3-bit priorities.
//
// Implemented standard regions needed by a simple RISC-V PLIC driver:
//   priority[N]       base + 0x000004*N
//   pending           base + 0x001000
//   enable hart0 M    base + 0x002000
//   threshold hart0 M base + 0x200000
...
module plic (
    ...
    always @(*) begin
        ...
        for (k_comb = 1; k_comb <= 16; k_comb = k_comb + 1) begin
    ...
    always @(*) begin
        bus_rdata = 32'd0;
        if (bus_addr >= 22'h000004 && bus_addr <= 22'h000040 && bus_addr[1:0] == 0) begin
            if (priority_index >= 1 && priority_index <= 16)
                bus_rdata = {29'd0,source_priority[priority_index]};
        end else begin
            case (bus_addr)
                22'h001000: bus_rdata = {15'd0,pending,1'b0};
                22'h002000: bus_rdata = {15'd0,enable,1'b0};
                22'h200000: bus_rdata = {29'd0,threshold};
                22'h200004: bus_rdata = {27'd0,best_id};
    ...
    always @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            ...
            enable    <= 16'd0;
            threshold <= 3'd0;
            for (k_seq = 1; k_seq <= 16; k_seq = k_seq + 1)
            ...
            if (bus_valid && |bus_wstrb) begin
                if (bus_addr >= 22'h000004 && bus_addr <= 22'h000040 && bus_addr[1:0] == 0) begin
                    if (priority_index >= 1 && priority_index <= 16 && bus_wstrb[0])
                        source_priority[priority_index] <= bus_wdata[2:0];
                end else begin
                    case (bus_addr)
                        22'h002000: begin
                            if (bus_wstrb[0]) enable[6:0]  <= bus_wdata[7:1];
                            if (bus_wstrb[1]) enable[14:7] <= bus_wdata[15:8];
                            if (bus_wstrb[2]) enable[15]   <= bus_wdata[16];
                        end
                        ...
                        22'h200004: begin
                            ...
                            if (bus_wdata[4:0] >= 1 && bus_wdata[4:0] <= 16)
                                in_service[bus_wdata[4:0]-1] <= 1'b0;
                        end
                        default: ;
                    endcase
                end
