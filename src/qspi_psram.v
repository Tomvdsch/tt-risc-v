// -----------------------------------------------------------------------------
// qspi_psram.v
// APS6404L-compatible 8 MiB SPI/quad-I/O PSRAM controller.
//
// v0.2.30 follows the proven KianV TT06 physical transaction protocol:
//   * the memory remains in ordinary SPI mode (there is no persistent QPI mode)
//   * command byte 0x38/0xEB is transmitted serially on DQ0
//   * the 24-bit address and data use all four DQ pins
//   * a read has six dummy clocks before quad data
//
// The interface and burst output are Tiny32-specific. This is an independent
// implementation informed by KianV's qqspi.v, copyright (c) 2022 hirosh dabui,
// used under its permissive ISC-style notice. See docs/KIANV_PSRAM_ADAPTATION.md.
//
// Reset recovery first sends 0xF5 as a quad command. It is ignored if the RAM
// was already in SPI mode, but returns a still-powered device from QPI to SPI.
// Serial reset-enable/reset (0x66/0x99) then establishes a known SPI state.
// SCK idles high and data is sampled on rising edges, matching KianV's mode-3
// sequencing. One controller clock is one SCK half-period.
// -----------------------------------------------------------------------------
module qspi_psram #(
    parameter POWERUP_WAIT_CYCLES = 6400,
    parameter RESET_WAIT_CYCLES   = 160000,
    parameter INTEROP_WAIT_CYCLES = 2
) (
    input  wire        clk,
    input  wire        rst_n,
    ...
    always @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            state              <= ST_POWER;
            wait_count         <= POWERUP_WAIT_CYCLES;
            req_done           <= 1'b0;
            rd_valid           <= 1'b0;
            rd_data            <= 32'd0;
            init_done          <= 1'b0;
            ...
            case (state)
                ...
                // KianV's key distinction: the command is one-bit SPI even
                // though the following address and payload are quad transfers.
                ST_CMD: begin
                    if (ram_sck) begin
                        ram_sck <= 1'b0;
                    end else begin
                ...
                ST_READ: begin
                    ...
                    if (ram_sck) begin
                        ...
                        case (read_nibble_index)
                            ...
                            3'd1: read_word[7:0] <= {read_hi_nibble,ram_dq_i};
                            3'd2: read_hi_nibble <= ram_dq_i;
                            3'd3: read_word[15:8] <= {read_hi_nibble,ram_dq_i};
                            3'd4: read_hi_nibble <= ram_dq_i;
                            3'd5: read_word[23:16] <= {read_hi_nibble,ram_dq_i};
                            3'd6: read_hi_nibble <= ram_dq_i;
                            default: begin
                                read_word[31:24] <= {read_hi_nibble,ram_dq_i};
                                rd_data  <= {{read_hi_nibble,ram_dq_i},read_word[23:0]};
                                rd_valid <= 1'b1;
                            end
                        endcase
                ...
                ST_FINISH: begin
                    ...
                    ram_sck    <= 1'b1;
                    ram_dq_oe  <= 4'd0;
                    wait_count <= INTEROP_WAIT_CYCLES;
                    state      <= ST_INTEROP;
                end
