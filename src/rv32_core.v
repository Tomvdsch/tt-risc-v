// -----------------------------------------------------------------------------
// rv32_core.v
// Tiny multi-cycle RV32I core with optional iterative RV32M/RV32A,
// Zicsr, Zifencei, and minimal Machine/User privilege modes.
//
// Design goals:
//   * small enough to be realistic in a 16-tile TinyTapeout design
//   * simple valid/ready memory bus
//   * enough machine-mode architectural support for bare metal and Zephyr
//   * no pipeline, no branch predictor, no MMU/PMP
//
// This is deliberately a simple core: only one memory transaction can be in
// flight, and most instructions take multiple clocks. External serial PSRAM is
// the dominant performance limit anyway, so area is more valuable than IPC.
// -----------------------------------------------------------------------------
module rv32_core #(
    ...
    // Useful on FPGA/simulation; synthesis removes it if unconnected.
    ...
    wire [31:0] rs1_val = (rs1_idx == 0) ? 32'd0 : regs[rs1_idx];
    wire [31:0] rs2_val = (rs2_idx == 0) ? 32'd0 : regs[rs2_idx];
    ...
    generate
        if (ENABLE_M) begin : gen_mdu
            rv32_mdu u_mdu (
                ...
                .op_a(mdu_a), .op_b(mdu_b), .busy(mdu_busy_i),
                .done(mdu_done_i), .result(mdu_result_i)
            );
        end else begin : gen_no_mdu
            assign mdu_busy_i   = 1'b0;
    ...
    task write_csr;
        ...
        begin
            case (addr)
                ...
                12'h306: csr_mcounteren <= value & 32'h0000_0007;
                    ...
                    case (opcode)
                        ...
                        7'b0010011: begin // OP-IMM
                            case (funct3)
                                ...
                                3'b101: alu_result = instr[30] ? ($signed(rs1_val) >>> instr[24:20])
                                                               : (rs1_val >> instr[24:20]);
                        ...
                        7'b0110011: begin // OP and RV32M
                            if (funct7 == 7'b0000001) begin
                                ...
                                case (funct3)
                                    ...
                                    3'b101: alu_result = funct7[5] ? ($signed(rs1_val) >>> rs2_val[4:0])
                                                                  : (rs1_val >> rs2_val[4:0]);
