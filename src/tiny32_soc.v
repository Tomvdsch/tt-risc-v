// -----------------------------------------------------------------------------
// tiny32_soc.v
// Top-level SoC logic, independent of TinyTapeout or FPGA packaging.
//
// Memory map:
//   8000_0000 - 807F_FFFF   8 MiB external APS6404L PSRAM
//   0200_0000 - 0200_FFFF   CLINT-compatible machine timer/software IRQ
//   0C00_0000 - 0C3F_FFFF   compact PLIC-compatible interrupt controller
//   1000_0000 - 1000_0FFF   system information
//   1000_1000 - 1000_1FFF   GPIO + pin mux
//   1000_2000 - 1000_2FFF   UART
//   1000_3000 - 1000_3FFF   SPI master
//   1000_4000 - 1000_4FFF   I2C master
//   1000_5000 - 1000_5FFF   PWM
//   1000_6000 - 1000_60FF   timer 0
//   1000_6100 - 1000_61FF   timer 1
//   1000_7000 - 1000_7FFF   watchdog
//   2000_0000 - 207F_FFFF   read-only external W25Q64 flash window
// -----------------------------------------------------------------------------
module tiny32_soc #(
    parameter ENABLE_M = 1,
    parameter ENABLE_A = 1,
    parameter ENABLE_U = 1,
    parameter ENABLE_COUNTERS = 1,
    parameter ICACHE_WORDS = 4,
    parameter ICACHE_LINE_BITS = 2,
    parameter SYS_CLK_HZ = 32000000,
    ...
    // -------------------------------------------------------------------------
    // External flash controller. The CPU executes the small stage-1 loader
    // directly from this read-only window after PSRAM initialization.
    // -------------------------------------------------------------------------
    ...
    // -------------------------------------------------------------------------
    // External PSRAM controller, instruction cache, and data bridge.
    // -------------------------------------------------------------------------
    ...
    reg [31:0] data_ram_store_data;
    reg [3:0] data_ram_store_wstrb;
    ...
    wire runtime_flash_rd_valid = flash_ctrl_rd_valid && (flash_owner == 1'b1);
    ...
    qspi_psram #(
        ...
        // deselect time at the 32 MHz tapeout target without crippling traffic.
    ...
    // Hardware boot work is now only PSRAM initialization. The flexible image
    // parser/copy/CRC logic lives in the replaceable flash stage-1 program.
    ...
    icache #(.LINE_WORDS(ICACHE_WORDS), .LINE_BITS(ICACHE_LINE_BITS)) u_icache (
    ...
    // Merge partial CPU stores with the existing aligned PSRAM word. Real-board
    // testing showed that the attached APS6404L path accepts four-byte writes
    // reliably but does not latch the controller's shorter write transactions.
    // Keeping the physical write at four bytes also gives the CPU correct SB/SH
    // behavior without overwriting untouched neighboring lanes.
    always @(*) begin
        data_ram_merged_data = data_ram_rdata;
        if(data_ram_store_wstrb[0])
            data_ram_merged_data[7:0] = data_ram_store_data[7:0];
        if(data_ram_store_wstrb[1])
            data_ram_merged_data[15:8] = data_ram_store_data[15:8];
        if(data_ram_store_wstrb[2])
            data_ram_merged_data[23:16] = data_ram_store_data[23:16];
        if(data_ram_store_wstrb[3])
            data_ram_merged_data[31:24] = data_ram_store_data[31:24];
    end

    // Data-side PSRAM bridge. Reads are aligned to 32-bit words because the CPU
    // performs byte/halfword extraction after receiving the word. Full-word
    // stores go directly to PSRAM; partial stores use an aligned read followed
    // by a full-word write of the merged value.
    always @(posedge clk or negedge cpu_rst_n) begin
        if(!cpu_rst_n) begin
            ...
            data_ram_rdata<=0; data_ram_store_data<=0;
            data_ram_store_wstrb<=0; data_ram_ready<=1'b0;
        end else begin
            data_ram_ready<=1'b0;
                ...
                DR_IDLE: begin
                    ...
                    if(cpu_mem_valid && !cpu_mem_instr && sel_ram) begin
                        if((|cpu_mem_wstrb) && cpu_mem_wstrb != 4'b1111) begin
                            data_ram_store_data <= cpu_mem_wdata;
                            data_ram_store_wstrb <= cpu_mem_wstrb;
                            data_ram_write <= 1'b0;
                            data_ram_addr <= {cpu_mem_addr[22:2],2'b00};
                            data_ram_wdata <= 32'd0;
                            data_ram_wstrb <= 4'd0;
    ...
    always @(posedge clk or negedge cpu_rst_n) begin
        if(!cpu_rst_n) begin
            ...
            runtime_flash_rdata<=0; runtime_flash_ready<=1'b0;
        end else begin
            runtime_flash_ready<=1'b0;
            if(runtime_flash_rd_valid)
                runtime_flash_rdata<=flash_ctrl_rd_data;
    ...
    always @(*) begin
        ...
        if(cpu_mem_valid) begin
            if(sel_ram) begin
                ...
                if(cpu_mem_instr) begin cpu_mem_ready=ic_cpu_ready; cpu_mem_rdata=ic_cpu_rdata; end
                else begin cpu_mem_ready=runtime_flash_ready; cpu_mem_rdata=runtime_flash_rdata; end
            end else begin
