module tiny32_soc #(
    ...
    parameter ENABLE_SPI = 0,
    parameter ENABLE_I2C = 0,
    parameter ENABLE_TIMER1 = 0,
    parameter ENABLE_WATCHDOG = 0,
    parameter PWM_CHANNELS = 0,
    ...
    generate
        ...
        if (ENABLE_I2C) begin : gen_i2c
            ...
            assign i2c_rdata = 32'd0;
            assign i2c_scl_drive_low = 1'b0;
            assign i2c_sda_drive_low = 1'b0;
            assign i2c_irq = 1'b0;
        end

        if (PWM_CHANNELS > 0) begin : gen_pwm
            pwm #(.CHANNELS(PWM_CHANNELS)) u_pwm(
                .clk(clk),.rst_n(runtime_rst_n),.bus_valid(cpu_mem_valid && page_pwm),.bus_addr(cpu_mem_addr[7:0]),
                .bus_wdata(cpu_mem_wdata),.bus_wstrb(cpu_mem_wstrb),.bus_rdata(pwm_rdata),.pwm_out(pwm_out_i));
        end else begin : gen_no_pwm
            assign pwm_rdata = 32'd0;
            assign pwm_out_i = 5'd0;
        end
    endgenerate
    ...
    generate
        if (ENABLE_TIMER1) begin : gen_timer1
        ...
        end else begin : gen_no_timer1
            assign timer1_rdata = 32'd0;
            assign timer1_irq = 1'b0;
        end
        ...
        if (ENABLE_WATCHDOG) begin : gen_watchdog
            ...
            assign wdog_rdata = 32'd0;
            assign wdog_irq = 1'b0;
            assign watchdog_reset_pulse = 1'b0;
        end
    endgenerate
    ...
    always @(*) begin
        ...
        else if(ENABLE_SPI && page_spi) periph_rdata=spi_rdata;
        else if(ENABLE_I2C && page_i2c) periph_rdata=i2c_rdata;
        else if(PWM_CHANNELS > 0 && page_pwm) periph_rdata=pwm_rdata;
        else if(timer0_sel) periph_rdata=timer0_rdata;
        else if(ENABLE_TIMER1 && timer1_sel) periph_rdata=timer1_rdata;
        else if(ENABLE_WATCHDOG && page_wdog) periph_rdata=wdog_rdata;
