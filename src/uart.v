// -----------------------------------------------------------------------------
// uart.v - compact 8-N-1 UART, one-byte RX holding register, no FIFO.
// Base in SoC: 0x1000_2000
//   0x00 DATA    R: RX byte (clears RX valid), W: transmit byte
//   0x04 STATUS  bit0 TX ready, bit1 RX valid, bit2 RX overrun
//   0x08 CTRL    bit0 RX IRQ enable, bit1 TX-ready IRQ enable
//   0x0C DIV     system clocks per serial bit (278 ~= 115200 at 32 MHz)
// -----------------------------------------------------------------------------
module uart #(
    parameter DEFAULT_DIV = 278
) (
    input  wire        clk,
    input  wire        rst_n,
