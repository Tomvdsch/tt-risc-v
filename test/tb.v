...
`ifndef GL_TEST
    ...
    rv32_core #(.ENABLE_M(1), .ENABLE_A(0), .ENABLE_U(0), .ENABLE_COUNTERS(0)) cpu_unit (
        ...
        .time_value(cpu_time_value), .mem_valid(cpu_mem_valid), .mem_instr(cpu_mem_instr),
    ...
    reg mdu_test_start;
    reg [2:0] mdu_test_funct3;
    reg [31:0] mdu_test_a;
    reg [31:0] mdu_test_b;
    wire mdu_test_busy;
    wire mdu_test_done;
    wire [31:0] mdu_test_result;
    rv32_mdu mdu_unit (
        .clk(clk), .rst_n(unit_rst_n), .start(mdu_test_start),
        .funct3(mdu_test_funct3), .op_a(mdu_test_a), .op_b(mdu_test_b),
        .busy(mdu_test_busy), .done(mdu_test_done), .result(mdu_test_result)
    );
`endif
...
`ifndef GL_TEST
        ...
        cpu_time_value = 64'h1122_3344_5566_7788;
        ...
        mdu_test_start = 0; mdu_test_funct3 = 0; mdu_test_a = 0; mdu_test_b = 0;
`endif
    end
endmodule
